...
async function resolvePaths(claudeDir) {
  const projectRoot = path6.dirname(claudeDir);
  ...
  return {
    ...
    claudeDir,
    autoDir,
    remindersDirs: [path6.join(autoDir, "reminders")],
    validatorsDirs: [path6.join(autoDir, "validators")]
  };
}
async function resolvePathsFromEnv() {
  ...
  if (pluginRoot && pluginData) {
    ...
    const claudeDir2 = path6.join(projectRoot, ".claude");
    const projectAutoDir = path6.join(projectRoot, DEFAULT_AUTO_DIR);
    const autoDir = projectAutoDir;
    return {
      projectRoot,
      claudeDir: claudeDir2,
      autoDir,
      remindersDirs: [path6.join(pluginRoot, "reminders"), path6.join(projectAutoDir, "reminders")],
      validatorsDirs: [path6.join(pluginRoot, "validators"), path6.join(projectAutoDir, "validators")]
    };
  }
...
function loadReminders(dirs, context, overrides) {
  ...
  for (const dir of dirs) {
    ...
    for (const filename of filenames) {
      ...
      seen.add(filename);
      const content = fs6.readFileSync(path7.join(dir, filename), "utf8");
...
async function handlePreToolUse(claudeDirOrPaths, sessionId, toolInput, options2 = {}) {
