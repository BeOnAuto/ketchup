...
async function resolvePaths(claudeDir) {
  const projectRoot = path4.dirname(claudeDir);
  ...
  return {
    ...
    claudeDir,
    autoDir,
    remindersDirs: [path4.join(autoDir, "reminders")],
    validatorsDirs: [path4.join(autoDir, "validators")]
  };
}
async function resolvePathsFromEnv() {
  ...
  if (pluginRoot && pluginData) {
    ...
    const claudeDir2 = path4.join(projectRoot, ".claude");
    const projectAutoDir = path4.join(projectRoot, DEFAULT_AUTO_DIR);
    const autoDir = projectAutoDir;
    return {
      projectRoot,
      claudeDir: claudeDir2,
      autoDir,
      remindersDirs: [path4.join(pluginRoot, "reminders"), path4.join(projectAutoDir, "reminders")],
      validatorsDirs: [path4.join(pluginRoot, "validators"), path4.join(projectAutoDir, "validators")]
    };
  }
