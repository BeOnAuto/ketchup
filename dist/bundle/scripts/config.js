...
function resolveClaudeDirFromScript(scriptDir) {
  const projectRoot = path3.resolve(scriptDir, "..", "..");
  return path3.join(projectRoot, ".claude");
}
async function resolvePaths(claudeDir) {
  const projectRoot = path3.dirname(claudeDir);
  ...
  return {
    ...
    claudeDir,
    autoDir,
    remindersDirs: [path3.join(autoDir, "reminders")],
    validatorsDirs: [path3.join(autoDir, "validators")]
  };
}
async function resolvePathsFromEnv() {
  ...
  if (pluginRoot && pluginData) {
    ...
    const claudeDir2 = path3.join(projectRoot, ".claude");
    const projectAutoDir = path3.join(projectRoot, DEFAULT_AUTO_DIR);
    const autoDir = projectAutoDir;
    return {
      projectRoot,
      claudeDir: claudeDir2,
      autoDir,
      remindersDirs: [path3.join(pluginRoot, "reminders"), path3.join(projectAutoDir, "reminders")],
      validatorsDirs: [path3.join(pluginRoot, "validators"), path3.join(projectAutoDir, "validators")]
    };
  }
