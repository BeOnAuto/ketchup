...
var require_resolve_from = __commonJS({
  "node_modules/.pnpm/resolve-from@4.0.0/node_modules/resolve-from/index.js"(exports2, module2) {
    ...
    var resolveFrom = (fromDir, moduleId, silent) => {
      ...
      const fromFile = path8.join(fromDir, "noop.js");
...
var require_import_fresh = __commonJS({
  "node_modules/.pnpm/import-fresh@3.3.1/node_modules/import-fresh/index.js"(exports2, module2) {
    ...
    module2.exports = (moduleId) => {
      ...
      const cwd = parentPath ? path8.dirname(parentPath) : __dirname;
...
${error.message}`;
    ...
    function resolveTsConfig(directory) {
      ...
      if (filePath !== void 0) {
        const { config, error } = typescript.readConfigFile(filePath, (path8) => typescript.sys.readFile(path8));
        if (error) {
...
var require_util = __commonJS({
  "node_modules/.pnpm/cosmiconfig@9.0.0_typescript@5.9.3/node_modules/cosmiconfig/dist/util.js"(exports2) {
    ...
    function getPropertyByPath(source, path8) {
      if (typeof path8 === "string" && Object.prototype.hasOwnProperty.call(source, path8)) {
        return source[path8];
      }
      const parsedPath = typeof path8 === "string" ? path8.split(".") : path8;
    ...
    async function isDirectory(path8) {
      try {
        const stat = await fs_1.promises.stat(path8);
    ...
    function isDirectorySync(path8) {
      try {
        const stat = fs_1.default.statSync(path8);
        return stat.isDirectory();
      } catch (e) {
        if (e.code === "ENOENT") {
          return false;
...
${[...importStack, fullPath].map((path8, i) => `${i + 1}. ${path8}`).join("\n")} (same as ${idx + 1}.)`);
          }
        }
      }
...
var require_Explorer = __commonJS({
  "node_modules/.pnpm/cosmiconfig@9.0.0_typescript@5.9.3/node_modules/cosmiconfig/dist/Explorer.js"(exports2) {
    ...
    var Explorer = class extends ExplorerBase_js_1.ExplorerBase {
      ...
      async #fileExists(path8) {
        try {
          await promises_1.default.stat(path8);
          return true;
        } catch (e) {
          return false;
        }
...
var require_dist = __commonJS({
  "node_modules/.pnpm/cosmiconfig@9.0.0_typescript@5.9.3/node_modules/cosmiconfig/dist/index.js"(exports2) {
    ...
    function getResolvedSearchPlaces(moduleName, toolDefinedSearchPlaces, userConfiguredOptions) {
      const userConfiguredSearchPlaces = userConfiguredOptions.searchPlaces?.map((path8) => path8.replace("{name}", moduleName));
...
function resolveClaudeDirFromScript(scriptDir) {
  const projectRoot = path5.resolve(scriptDir, "..", "..");
  return path5.join(projectRoot, ".claude");
}
async function resolvePaths(claudeDir) {
  const projectRoot = path5.dirname(claudeDir);
  const config = await loadConfig(projectRoot);
  const autoDirName = config.autoDir ?? DEFAULT_AUTO_DIR;
  const autoDir = path5.join(projectRoot, autoDirName);
  return {
    projectRoot,
    claudeDir,
    autoDir,
    remindersDirs: [path5.join(autoDir, "reminders")],
    validatorsDirs: [path5.join(autoDir, "validators")]
  };
}
async function resolvePathsFromEnv() {
  ...
  if (pluginRoot && pluginData) {
    ...
    const claudeDir2 = path5.join(projectRoot, ".claude");
    const projectAutoDir = path5.join(projectRoot, DEFAULT_AUTO_DIR);
    const autoDir = projectAutoDir;
    return {
      projectRoot,
      claudeDir: claudeDir2,
      autoDir,
      remindersDirs: [path5.join(pluginRoot, "reminders"), path5.join(projectAutoDir, "reminders")],
      validatorsDirs: [path5.join(pluginRoot, "validators"), path5.join(projectAutoDir, "validators")]
    };
  }
...
function loadReminders(dirs, context, overrides) {
  ...
  for (const dir of dirs) {
    ...
    for (const filename of filenames) {
      ...
      seen.add(filename);
      const content = fs5.readFileSync(path6.join(dir, filename), "utf8");
...
async function handleUserPromptSubmit(claudeDirOrPaths, sessionId, prompt) {
